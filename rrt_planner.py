...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        if s_nearest.euclidean_distance(s_rand)<=max_radius:
            ...
            vector = [s_rand.x-s_nearest.x, s_rand.y-s_nearest.y]
            v = np.array(vector)
            norm = np.linalg.norm(v)
            if (norm!=0):
                v = v/norm
            x=int(s_nearest.x+max_radius*v[0])
            y=int(s_nearest.y+max_radius*v[1])
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            ...
            s_new = self.steer_towards(s_from, s_to,s_from.euclidean_distance(s_to))
            interpolated = float(i)/max_checks * s_from.euclidean_distance(s_new)
            vector = [s_new.x-s_from.x, s_new.y-s_from.y]
            v = np.array(vector)
            norm = np.linalg.norm(v)
            if (norm!=0):
                v = v/norm
            x=int(s_from.x+interpolated*v[0])
            y=int(s_from.y+interpolated*v[1])
            s_away = State(x,y,s_from)
            if(self.state_is_free(s_away)):
                pass
            else:
                return False
