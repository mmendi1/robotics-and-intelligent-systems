...
class AStarPlanner(object):
    ...
    def plan(self, start_state, dest_state):
        ...
        Q = priority_dict()
        Q[start_state] = max(abs(start_state.x-dest_state.x),abs(start_state.y-dest_state.y))
        ...
        # Array that contains lower bound on cost of path from source to destination
        lower_bound = float("inf") * np.ones((world.shape[1], world.shape[0]))
        lower_bound[start_state.x,start_state.y]= max(abs(start_state.x-dest_state.x),abs(start_state.y-dest_state.y))
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                if (ns not in Q):
                    Q.__setitem__(ns,alternative_best_dist_ns + max(abs(ns.x-dest_state.x),abs(ns.y-dest_state.y)))
                    best_dist_found[ns.x, ns.y] = alternative_best_dist_ns
                    parents[ns] = s
                elif (alternative_best_dist_ns < best_dist_found[ns.x, ns.y]):
                    Q[ns] = alternative_best_dist_ns + max(abs(ns.x-dest_state.x),abs(ns.y-dest_state.y))
                    best_dist_found[ns.x, ns.y] = alternative_best_dist_ns
                    parents[ns] = s
